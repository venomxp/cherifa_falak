...
export const getNumerologyReport = async (name: string, dob: string, gematriaValue: number): Promise<string> => {
  const prompt = `Provide a mystical and insightful personality analysis in Arabic based on Numerology and Gematria (حساب الجُمَّل).
...
export const getLoveCompatibilityAnalysis = async (name1: string, name2: string, percentage: number): Promise<string> => {
    const prompt = `Provide a romantic compatibility analysis for two people, ${name1} and ${name2}.
...
export const getZodiacCompatibilityAnalysis = async (sign1: string, sign2: string): Promise<string> => {
    const prompt = `Provide a detailed zodiac compatibility analysis in Arabic between ${sign1} and ${sign2}.
...
export const getTarotInterpretationStream = async (cardName: string) => {
    const prompt = `Act as a wise and intuitive tarot reader. Provide a mystical and insightful interpretation in Arabic for the Tarot card: "${cardName}".
